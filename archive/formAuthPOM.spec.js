...
test.describe('Landing Page validation', () => {
  ...
  test('Validate expected link text on the landing page ', async ({ page }) => {
    ...
    await expect.soft(page.locator(landingPage.getABTesting)).toHaveText('A/B Testing');
    await expect.soft(page.locator(landingPage.getAddRemoveElement)).toHaveText('Add/Remove Elements');
